...
```console
0 14 * * * python /path/to/en_manual.py /path/to/config.json > /tmp/en_log.txt 2>&1
```
This ensures the script is run every day at 14:00 o'clock, and the scripts output - both standard and error output (`2>&1`) - are piped into a textfile `/tmp/en_log.txt`.
